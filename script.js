...
(function loadSavedQuestions() {
  try {
    const saved = localStorage.getItem('loihaylam_questions');
    if (saved) {
      const parsed = JSON.parse(saved);
      if (Array.isArray(parsed) && parsed.length >= 10) {
        cards.length = 0;
        parsed.forEach(q => cards.push(q));
      }
    }
  } catch(e) {}
})();
...
function saveQuestions() {
  try { localStorage.setItem('loihaylam_questions', JSON.stringify(cards)); } catch(e) {}
}
...
const cardBack = document.querySelector('.game-container .card-back');
...
document.getElementById("toggle-list")?.addEventListener("click", () => {
  document.getElementById("question-manager")?.classList.toggle("hidden");
});
...
(function() {
  ...
  const listDisplay = document.getElementById('player-list-display');
  if (!addBtn) return;
  ...
  function renderPlayers() {
    ...
    players.forEach((name, i) => {
      ...
      span.className = 'player-chip' + (i === currentIndex ? ' active' : '');
      span.textContent = name;
      listDisplay.appendChild(span);
    });
    nameDisplay.textContent = players.length ? players[currentIndex] : '—';
  }
