...
const {
  ...
  checkWinCondition, restartGame, tallyVotes, resolveNightKills,
...
function resolveAndAdvanceFromNight(roomId, roomSockets) {
  ...
  const { eliminated } = resolveNightKills(room);
  ...
  if (winner) {
    updateRoom(roomId, { phase: 'ended', winner, eliminated_last: eliminated || null });
  } else {
...
function buildStateFor(roomId, playerId) {
  ...
  return {
    ...
    myRole: me?.role || null,
    myRoleDesc,
    wolves: (isWolf || !!me?.is_host || room.phase === 'ended') ? wolves : [],
...
function handleWsMessage(msg, ws, roomSockets, roomId, playerId) {
  ...
  switch (type) {
    ...
    case 'night_vote': {
      ...
      if (result.error) { send(ws, 'error', { message: result.error }); return; }
      const players = getPlayers(roomId);
      appendLog(roomId, { phase: 'night', action: 'wolf_vote', actorName: players.find((p) => p.id === playerId)?.name, targetName: players.find((p) => p.id === payload.targetId)?.name });
      broadcast(roomSockets, roomId);
      break;
    }
    ...
    case 'end_night': {
      const room = getRoom(roomId);
      if (!room || room.host_player_id !== playerId) {
        send(ws, 'error', { message: 'Only the host can end the night' }); return;
      }
      if (room.phase !== 'night') { send(ws, 'error', { message: 'Not in night phase' }); return; }
    ...
    case 'skip_night_action': {
      ...
      const actions = JSON.parse(room.night_actions || '{}');
      if (me.role === 'seer') {
        actions.seerSkipped = true;
        appendLog(roomId, { phase: 'night', action: 'seer_skip', actorName: me.name });
      } else if (me.role === 'bodyguard') {
        actions.bodyguardSkipped = true;
        appendLog(roomId, { phase: 'night', action: 'bodyguard_skip', actorName: me.name });
      } else return;
      updateRoom(roomId, { night_actions: JSON.stringify(actions) });
      broadcast(roomSockets, roomId);
      break;
    ...
    case 'custom_action': {
      ...
      const players = getPlayers(roomId);
      const me = players.find((p) => p.id === playerId);
      if (!me || !me.is_alive) { send(ws, 'error', { message: 'Not in game' }); return; }
      const roleConfig = JSON.parse(room.role_config || '{}');
      const customRole = (roleConfig.customRoles || []).find((r) => r.name === me.role);
      if (!customRole || !customRole.actions?.includes(payload.actionName)) {
        send(ws, 'error', { message: 'Invalid action' }); return;
      }
      appendLog(roomId, { phase: room.phase, action: 'custom_action', actorName: me.name, detail: payload.actionName });
      broadcast(roomSockets, roomId);
      break;
    }
